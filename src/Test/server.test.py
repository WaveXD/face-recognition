#แก้โค้ดให้กล้องบันทึกรูปทุกวินาที เพื่อเอารูปนั้นไป live
...
def capture_and_save_images(camera, camera_id):
    while True:
        ...
        filename = f"{camera_id}/{camera_id}.jpg"
        _, img_encoded = cv2.imencode('.jpg', frame)
        img_bytes = img_encoded.tobytes()
        blob = bucket.blob(filename)

        # Set cache control and upload the image to Google Cloud Storage
        ...
        print(f"Image updated for {camera_id}")
        time.sleep(10)  # Delay for 2 seconds

# Run capturing in separate threads for each camera
Thread(target=capture_and_save_images, args=(camera1, 'Camera1')).start()
Thread(target=capture_and_save_images, args=(camera2, 'Camera2')).start()
...
def generate_frames_from_images():
    while True:
        filenames = ["Camera1/Camera1.jpg", "Camera2/Camera2.jpg"]
        frames = []
        for filename in filenames:
            blob = bucket.blob(filename)
            try:
                # Force browsers to perceive this as a new image
        ...
        time.sleep(10)  # Keep this to sync with image updates
...
def video_stream():
    return Response(generate_frames_from_images(), mimetype='multipart/x-mixed-replace; boundary=frame')
